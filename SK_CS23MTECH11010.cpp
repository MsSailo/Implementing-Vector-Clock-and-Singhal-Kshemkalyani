...
int n, m, lambda, received_msg = 0, msg_size = 0;
float alpha;
...
void func_recv(int socket_file_descriptor, int id, vector<int> &vect_clock, vector<int> &ls, vector<int> &lu)
{
	...
	{
		int recv_int[n+1][2], client_socket;
		int message_number = recv_int[0][0];
		int client_id = recv_int[0][1];
		...
		if(success > 0)
		{
			...
			for(int i = 1;i<n+1;++i)
			{
				if(recv_int[i][0]>-1)
				{
					int index = recv_int[i][0];
					int value = recv_int[i][1];
					// received tuple updated the vector clock
					if(vect_clock[index]<value)
					{
						vect_clock[index] = value;
						lu[index] = vect_clock[id];
					}
				}
				else
					break;
...
void func_event(int socket_file_descriptor, int id, vector<int> &vect_clock, vector<int> &ls, vector<int> &lu)
{
	...
	for(int i = 0; i < ceil(total_events); i++)
	{
		...
		else
		{
			...
			{
				if(lu[k] > ls[client_id])
					update.push_back(k);
			}
			
			lock_mutex.lock();
			msg_size += (update.size()+1)*2;
			lock_mutex.unlock();

			int send_msg_3[update.size()+1][2];
			send_msg_3[0][0] = message;
			send_msg_3[0][1] = id;
			for(int i = 1;i<update.size()+1;++i)
			{	
				send_msg_3[i][0] = update[i-1];
				send_msg_3[i][1] = vect_clock[update[i-1]];
				
			}

			while(send(send_socket, send_msg_3, sizeof(send_msg_3), 0 ) < 0);
			close(send_socket);
...
void func_thread(int id)
{
	...
	thread thread_event = thread(func_event, socket_file_descriptor, id, ref(vect_clock), ref(ls), ref(lu)); 
    thread thread_recv = thread(func_recv, socket_file_descriptor, id, ref(vect_clock), ref(ls), ref(lu));
...
int main()
{
	...
	output_file_descriptor = fopen("SK-log.txt","w+");
